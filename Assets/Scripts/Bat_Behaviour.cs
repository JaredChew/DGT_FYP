...
public class Bat_Behaviour : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if (lookingRight && Physics2D.Raycast(transform.position, Vector2.right, wallVisionDistance, wallObjects)) {
            flipEnemy();
        }
        else if (!lookingRight && Physics2D.Raycast(transform.position, Vector2.left, wallVisionDistance, wallObjects)) {
            flipEnemy();
        }

        //Flip when enemy sees other enemies
        if (lookingRight && Physics2D.Raycast(transform.position, Vector2.right, wallVisionDistance, enemyObjects)) {
            flipEnemy();
        }
        else if (!lookingRight && Physics2D.Raycast(transform.position, Vector2.left, wallVisionDistance, enemyObjects)) {
            flipEnemy();
        }

        //Check if enemy sees player
        if (lookingRight) {
            playerDetected = Physics2D.Raycast(transform.position, Vector2.right, playerVisionDistance, playerObject);
        }
        else {
            playerDetected = Physics2D.Raycast(transform.position, Vector2.left, playerVisionDistance, playerObject);
        }
        ...
        if (playerDetected && attackRecoveryCountdown <= 0) {
            ...
            //Attack player if within attack distance
            if (Physics2D.Raycast(transform.position, Vector2.right, attackVisionDistance, playerObject) || Physics2D.Raycast(transform.position, Vector2.left, attackVisionDistance, playerObject)) {
                ...
                /*
                if (lookingRight) { enemyRigidbody.AddForce(new Vector2(pushForce, 0)); }
                else { enemyRigidbody.AddForce(new Vector2(-pushForce, 0)); }
                */
            }
    ...
    */
    ...
    private void addForce() {
        enemyRigidbody.AddForce(new Vector2(lookingRight ? pushForce : -pushForce, 0));
    }

...
public class SkeletonBrute_Behaviour : MonoBehaviour {
    ...
    [SerializeField] private LayerMask turnBackObjects;
    ...
    void FixedUpdate() {
        ...
        //Flip when enemy sees special invisible object
        if (lookingRight && Physics2D.Raycast(eyes.position, Vector2.right, wallVisionDistance, turnBackObjects))
        {
            flipEnemy();
        }
        else if (!lookingRight && Physics2D.Raycast(eyes.position, Vector2.left, wallVisionDistance, turnBackObjects))
        {
            flipEnemy();
        }
        ...
        //Debug.Log(attackRecoveryCountdown);
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        ...
        if (collision.CompareTag(Global.gameObjectTag_HeavyAttack)) {
            health--;
        }
    ...
    private void OnCollisionEnter2D(Collision2D collision) {
        ...
        if (collision.collider.CompareTag(Global.gameObjectTag_HeavyAttack)) {
            health--;
        }
